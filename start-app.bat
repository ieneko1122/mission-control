...
rem バックエンド(18080)とフロント(5173)を別ウィンドウで起動し、
...
echo [Mission Control] Backend (Spring Boot / 18080) を起動します...
...
powershell -NoProfile -Command "for($i=0;$i -lt 90;$i++){try{Invoke-WebRequest -Uri 'http://localhost:18080/api/status' -UseBasicParsing -TimeoutSec 2 | Out-Null; exit 0}catch{Start-Sleep -Seconds 1}}"
